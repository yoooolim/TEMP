...
class Realtime_engine {
    ...
    fun Real_engine(naverMap: NaverMap, location: Location) {
        ...
        if (timestamp <= 3) {
            ...
            printMatched(subMatching, Color.GREEN,  naverMap) // 매칭: 초록색
            subMatching.clear()
        ...
        if (crossroad_check == 1) {
            if (subGPSs.size == 5) { // 5초후까지 확인
                ...
                crossroad_check = 0
                printMatched(subMatching, Color.GREEN,naverMap) // 세정 매칭: 초록색
                subMatching.clear()
                subGPSs.clear()
                arrOfCandidates.clear()
                //clear
    ...
    fun printMatched(matched: ArrayList<Candidate>, COLOR: Int, naverMap: NaverMap) {
        for (i in matched.indices) { //indices 또는 index사용
            ...
            marker.height = 40
            // 마커가 너무 커서 크기 지정해줌
            marker.icon = OverlayImage.fromResource(R.drawable.matching_point)
